...
// Debug
unsigned long sensorStartTime = 0;    // Records the time when the sensor was read
const unsigned long sensorDuration = 200;      // Duration for debug
...
void loop() {
  ...
  // Debug sensor
  if (currentTime - sensorStartTime >= sensorDuration) {
    sensorStartTime = millis();  
    if (digitalRead(garageSensor1) == HIGH) {
      Serial.println("Garage open (garageSensor1 is HIGH)");
    } else {
      Serial.println("Garage closed (garageSensor1 is LOW)");
    }
  }
